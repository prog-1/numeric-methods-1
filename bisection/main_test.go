...
func TestSolveBisect(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"4", 1, 5, func(x float64) float64 { return 3*math.Pow(x, 4) - 4*math.Pow(x, 3) - 12*x*x - 5 }, 2.824},
		{"5", 2, 7, func(x float64) float64 { return 3*math.Pow(x, 4) - 4*math.Pow(x, 3) - 12*x*x - 5 }, 2.824},
		{"6", -5, -1, func(x float64) float64 { return 3*math.Pow(x, 4) - 4*math.Pow(x, 3) - 12*x*x - 5 }, -1.592},
		{"7", 1.5, 4.5, func(x float64) float64 { return x - 5*math.Sin(x) - 3.5 }, 3.201},
		{"8", -1, 2, func(x float64) float64 { return (1+x)*math.Pow(math.E, 3*x) - 5 }, 0.42},
		{"9", -3, 0, func(x float64) float64 { return (1+x)*math.Pow(math.E, (-2)*x) - (-8) }, -1.445},
	} {
		t.Run(tc.name, func(t *testing.T) {
			got := SolveBisect(tc.a, tc.b, eps, tc.f)
